#pragma once
#include<iostream>
#include<algorithm>
#include<vector>
#include "Model.h"
#include "View.h"
using namespace std;

void Init(struct Managing& managing);
void signUp(struct Managing& managing, const string name, const string address, int phoneNumber, int money, const string ID, const string password, const string gender);
void signIn(struct Managing& managing, const string ID, const string password);
void signOut(struct Managing& managing);
void registerProduct(struct Managing& managing, const string name, const string category, int price, int spare);
void inputBasket(struct Managing& managing, int index);
void purchase(struct Managing& managing);